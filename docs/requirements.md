...
| GET | `/api/languages` | 🟡 (v1.43) The enabled languages the signed-in account may use, each with its `track` and `kind`; the Japanese pools only for an account whose display language is Japanese (§13.11) |
...
🟡 (v1.13) Every route requires a signed-in session unless it is explicitly public; the public routes are the health checks and register, login, and logout (🟡 v1.43: `GET /api/languages` is signed-in only, §13.11). Errors use one body shape, `{ statusCode, error, message }`, and server errors never include their cause.
...
Initial size is 500 words, 200 sentences, and 60 paragraphs per language, added a language at a time. Both languages have it.
...
**Header and track menu.** The logo leads to Home. Beside it is a switch between the tracks offered, in the order Code, Japanese, English, with the current one marked (on the play screen, the track of the run; switching away from a run asks first, like every link of the header). The header keeps only what belongs to no track, the settings and the player. A track's own screens (Play, CPU battles, High scores, Status, Play log) are a menu under the header, with its name and the current screen marked in the track's colour. A natural-language pool is named by its kind (Words, Sentences, Paragraphs; 単語, 短文, 長文), since the track already says the language.
...
**Home.** A card for each track offered, in the order Code, Japanese, English: its name in its colour, the rank icon and name, the overall rating of that track (§13.3), and the bar towards the next rank; the card leads to the track. Below it, the play-history grid. The ratings and the play history are read separately, so one failing does not take the other, or the cards, with it.

**Play-history grid.** As on GitHub: one column per week, one row per day, the weeks starting on Sunday (§6.1), for the range `GET /api/activity` returns (the last 365 days by default), with the days before the range and after its end left blank, and labels for the months and for Monday, Wednesday, and Friday in the display language. A cell is coloured when a run was played that day, in four shades by the number of runs of that kind (1, 2 to 3, 4 to 6, 7 or more). Code runs are green and natural-language runs violet; a day with both shows both, the cell split along its diagonal. Colour is never the only signal (§9.6): each cell has a text title with its date and counts, the totals (runs, days played) are written above the grid, the picture is hidden from a screen reader, and the days with runs are listed in a table under the grid ("Show as a list"). A narrow window scrolls the grid sideways.
...
| 1.43 | Japanese pool names | 単語, 短文, 長文 in the Japanese interface (they were 単語, 1行, 複数行); the English names are unchanged |
| 1.43 | Japanese content rewritten | The first drafts read like translations: sentences began with 私は and words were two characters long (2.2 on average). The pools were rewritten as Japanese is written and said: sentences mostly without a subject, proverbs and set phrases, everyday remarks, notices and letters; words average about five characters (427 words, mean 5.3) with four-character idioms, katakana loanwords, greetings, and set phrases. Words and sentences are original writing in the common style of typing practice, not copied from any site. The block ids follow the text, so every Japanese block has a new id; runs already stored keep the ids they were played with |
